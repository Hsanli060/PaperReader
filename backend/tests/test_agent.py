...
FIX-4 注：Agent 核心收敛为 run_stream_async 后，涉及主循环的测试用
"asyncio.run 收集事件再断言"的方式（不依赖 pytest-asyncio 插件配置）。
"""
import asyncio
import json
...
def collect_events(coro_gen) -> list[dict]:
    """把 async 生成器的事件收进列表（同步测试里消费 async 流的标准写法）"""
    async def _collect():
